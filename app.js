...
function draw(){
    ...
    if (game.drawPile.length === 0) {
        ...
        // for(let i = 0; i<game.discPile.length; i++){  fun bug
            ...
            console.log(`Discpile: ${game.discPile.length}`);         
...
function battle() {
    ...
    console.log(`enemy dmg: ${game.enemy.dmg}, player block: ${game.player.blkAmount}`);
...
cardElements.forEach((card) => {
    card.addEventListener(`click`, () => {
        console.log(`passing cardid: ${card.id}, passing cardtext: ${card.textContent}`);
